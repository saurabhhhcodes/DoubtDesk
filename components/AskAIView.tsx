...
export default function AskAIView({
  ...
  useEffect(() => {
    if (initialDoubt) {
      setPrompt(
        initialDoubt.content === "Visual Inquiry" ? "" : initialDoubt.content,
      );
        ...
        <div className="p-12 bg-slate-900/40 border border-white/5 rounded-3xl flex flex-col items-center justify-center gap-4 text-center">
          ...
          <div>
            <p className="text-white font-black uppercase tracking-widest text-xs">
              Generating Video AI Masterpiece
            </p>
            <p className="text-slate-500 text-[10px] mt-1 italic">
              Creating slides, synthesizing voice, and rendering frames...
            </p>
          </div>
        </div>
      )}
      ...
      {videoUrl && (
        <div className="bg-slate-950 border-4 border-blue-500/30 rounded-[2.5rem] overflow-hidden shadow-2xl shadow-blue-500/10">
          <div className="bg-blue-500/10 px-6 py-4 border-b border-blue-500/20 flex items-center justify-between">
            ...
            <span className="bg-blue-500 text-white text-[9px] font-black px-3 py-1 rounded-full uppercase italic">
              4K UHD AI
            </span>
          </div>
            ...
            return (
              ...
              >
                ...
                >
                  {meta && (
                    <div
                      className={`flex items-center justify-center w-8 h-8 rounded-xl bg-white/5 border ${meta.badge}`}
                    >
                      {meta.icon}
                    </div>
                  )}
                  <h2 className="text-white font-black tracking-tight text-sm uppercase italic">
                    {sec.title}
                  </h2>
                  {idx === 0 && (
                    <button
